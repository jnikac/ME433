...
void ws2812b_setup() {
    ...
    TRISBbits.TRISB5 = 0; //set pin B7 as an output
    LATBbits.LATB5 = 0; //initially set pin B7 as off
}
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    for (i = 0; i < numLEDs; ++i) {
        ...
        for (j = 7; j >= 0; --j) {
            // if the bit is a 1
            if (((c[i].r >> j) & 0b1) == 0b1){ //this will take the red byte of LED 1 and shift it j bits to the right, and check if that is 1
                // the high is longer
        ...
        for (j = 7; j >= 0; --j) {
            // if the bit is a 1
            if (((c[i].g >> j) & 0b1) == 0b1){ //this will take the green byte of LED 1 and shift it j bits to the right, and check if that is 1
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
